...
<div class="nilai-container">
    ...
    <div class="card-simple" style="background-color: #e3f2fd; border-color: #1976d2;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <a href="{{ route('dosen.inputNilai.create') }}" class="btn-primary">
                Input Nilai Baru
            </a>
        </div>
    </div>
    ...
    <div class="card-simple">
        <form method="GET" action="{{ route('dosen.inputNilai.index') }}" id="filterForm">
    ...
    <div class="card-simple">
        ...
        @if($nilaiList->count() > 0)
                    ...
                    <tbody>
                        @foreach($nilaiList as $index => $nilai)
                            <tr>
                                ...
                                <td>{{ $nilai->prodi->nama_prodi ?? '-' }}</td>
                                <td>{{ $nilai->semester }}</td>
                                <td>{{ $nilai->tahun_ajaran }}</td>
                                <td>
                                    <span class="status-badge nilai-{{ strtolower(substr($nilai->nilai_huruf, 0, 1)) }}">
                                        {{ $nilai->nilai_angka }} ({{ $nilai->nilai_huruf }})
                                    </span>
                                </td>
                                <td>
                                    <div style="display: flex; gap: 4px; align-items: center;">
                                        <a href="{{ route('dosen.inputNilai.show', $nilai->id) }}" class="btn-success" title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('dosen.inputNilai.edit', $nilai->id) }}" class="btn-warning" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('dosen.inputNilai.destroy', $nilai->id) }}" method="POST" style="margin: 0;" onsubmit="return confirm('Yakin ingin menghapus nilai ini?')">
                                            @csrf
                                            @method('DELETE')
            ...
            <div style="margin-top: 24px; display: flex; justify-content: space-between; align-items: center;">
                <div style="color: #666; font-size: 14px;">
                    Menampilkan {{ $nilaiList->firstItem() ?? 0 }} - {{ $nilaiList->lastItem() ?? 0 }} dari {{ $nilaiList->total() }} data
                </div>
                <div>
        ...
        @else
            <div class="alert alert-warning">
                ...
                <br>
                <a href="{{ route('dosen.inputNilai.create') }}" class="btn-primary" style="margin-top: 12px;">
