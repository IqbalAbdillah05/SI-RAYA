...
@section('content')
    ...
    <div class="card-info" style="background: linear-gradient(135deg, #0B6623 0%, #0a5a1f 100%); border: none; padding: 24px; border-radius: 12px; margin-bottom: 20px; color: white;">
        <div>
            ...
            <p style="margin: 0; opacity: 0.95; font-size: 14px;">
                Jadwal perkuliahan semester {{ $selectedSemester }} - {{ $mahasiswa->prodi->nama_prodi ?? '-' }}
                @if(isset($krs))
                    <br>
                    <small style="opacity: 0.9;">
                        <i class="fas fa-info-circle"></i> Berdasarkan KRS yang telah Anda isi ({{ $krs->details->count() }} mata kuliah)
                    </small>
                @endif
            </p>
        </div>
    ...
    <div class="card-simple">
        <div style="background-color: #e8f5ec; padding: 20px; border-radius: 8px; margin-bottom: 0;">
            ...
            <form method="GET" action="{{ route('mahasiswa.jadwal.index') }}" id="filterForm">
    ...
    @if(!isset($hasKrs) || !$hasKrs)
        <div class="card-simple">
            <div class="empty-state">
                ...
                <h3>Belum Ada KRS</h3>
                <p>Anda belum mengisi KRS untuk semester {{ $selectedSemester }}.</p>
                <p class="empty-hint">Silakan isi KRS terlebih dahulu untuk melihat jadwal kuliah Anda.</p>
                <a href="{{ route('mahasiswa.krs.index') }}" class="btn-krs" style="display: inline-flex; align-items: center; gap: 8px; margin-top: 16px; padding: 12px 24px; background: linear-gradient(135deg, #0B6623 0%, #0a5a1f 100%); color: white; text-decoration: none; border-radius: 8px; font-weight: 600;">
                    <i class="fas fa-plus-circle"></i>
                    Isi KRS Sekarang
                </a>
            </div>
        </div>
    @elseif($jadwalList->count() > 0)
        <div class="card-simple" style="padding: 0;">
        @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $hari)
            @if(isset($jadwalByHari[$hari]) && $jadwalByHari[$hari]->count() > 0)
                <div class="day-section">
                    <div class="day-header">
                        <div class="day-badge {{ strtolower($hari) }}">
                            <i class="fas fa-calendar-alt"></i>
                            {{ $hari }}
                        </div>
                        <div class="day-count">
                            {{ $jadwalByHari[$hari]->count() }} Mata Kuliah
                        </div>
                    </div>
                            ...
                            <div class="schedule-card">
                                ...
                                <div class="schedule-body">
                                    <h3 class="schedule-title">
                                        {{ $jadwal->mataKuliah->nama_matakuliah ?? 'Mata Kuliah Tidak Ditemukan' }}
                                    </h3>
                                    
                                    <div class="schedule-meta">
                                        <div class="meta-item">
                                            <i class="fas fa-book-open"></i>
                                            <span>{{ $jadwal->mataKuliah->sks ?? '-' }} SKS</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="schedule-info">
                                        <div class="info-item">
                                            ...
                                            <div class="info-content">
                                                ...
                                                <div class="info-value">{{ $jadwal->dosen->nama_lengkap ?? 'Belum Ditentukan' }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
