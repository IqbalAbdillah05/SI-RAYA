...
@section('content')
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            <div>
                ...
                <p style="margin: 0; opacity: 0.95; font-size: 14px;">
                    Hasil studi semester {{ $selectedSemester }} - {{ $mahasiswa->prodi->nama_prodi ?? '-' }}
                </p>
            </div>
            @if($khs)
                <a href="{{ route('mahasiswa.khs.download', $selectedSemester) }}" class="btn-download" style="background: white; color: #0B6623; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 8px; transition: all 0.3s;">
    ...
    <div class="card-simple">
        <div style="background-color: #e8f5ec; padding: 20px; border-radius: 8px; margin-bottom: 0;">
            ...
            <form method="GET" action="{{ route('mahasiswa.khs.index') }}" id="filterForm">
        ...
        <div class="card-simple" style="padding: 0;">
            ...
            <div class="khs-summary">
                <h4 class="summary-title">
                    <i class="fas fa-info-circle"></i> Ringkasan Semester {{ $selectedSemester }}
                </h4>
                <div class="summary-grid">
                    <div class="summary-item">
                        <div class="summary-label">Tahun Ajaran</div>
                        <div class="summary-value">{{ $khs->tahun_ajaran ?? '-' }}</div>
                    </div>
                    <div class="summary-item">
                        <div class="summary-label">Total SKS</div>
                        <div class="summary-value">{{ $khs->total_sks ?? 0 }} SKS</div>
                    </div>
                    <div class="summary-item">
                        <div class="summary-label">IP Semester (IPS)</div>
                        <div class="summary-value {{ $khs->ips >= 3.0 ? 'text-success' : ($khs->ips >= 2.5 ? 'text-warning' : 'text-danger') }}">
                            {{ number_format($khs->ips ?? 0, 2) }}
                        </div>
                    </div>
                    <div class="summary-item">
                        <div class="summary-label">IP Kumulatif (IPK)</div>
                        <div class="summary-value {{ $khs->ipk >= 3.0 ? 'text-success' : ($khs->ipk >= 2.5 ? 'text-warning' : 'text-danger') }}">
                        ...
                        @foreach($khs->details as $index => $detail)
                            <tr>
                                ...
                                <td class="text-center">
                                    <span class="badge-nilai nilai-{{ strtolower($detail->nilai_huruf ?? 'e') }}">
                                        {{ $detail->nilai_huruf ?? '-' }}
                                    </span>
                                </td>
    ...
    @else
        <div class="card-simple">
            <div class="empty-state">
                ...
                <p>KHS untuk semester {{ $selectedSemester }} belum tersedia.</p>
